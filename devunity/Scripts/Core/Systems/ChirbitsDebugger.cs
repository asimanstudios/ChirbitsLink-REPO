...
{
    /// <summary>
    /// Herramienta de diagnóstico para el sistema de inputs móvil.
    /// Muestra en pantalla (OnGUI) el estado en tiempo real de:
    ///  - El servidor TCP (activo / puerto)
    ///  - Cada jugador: userId, GameObject asociado, IChibitsController encontrado
    ///
    /// SOLO activo en el Editor o en Development Builds. Se auto-desactiva en Release.
    /// Añadir este componente al mismo GameObject que TcpServer o a un Manager vacío.
    /// </summary>
    public class ChirbitsDebugger : MonoBehaviour
    {
        [Header("Visualización")]
        [Tooltip("Esquina de la pantalla donde aparece el panel HUD de debug")]
        public bool mostrarEnPantalla = true;

        [Tooltip("Tamaño de fuente del panel de debug")]
        ...
        private void Awake()
        {
            ...
            // En builds de release no queremos este overhead
            Destroy(this);
#else
            Debug.Log("[ChirbitsDebugger] Activado. Solo visible en Editor/Development Build.");
#endif
        }
